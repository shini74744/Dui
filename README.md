...
- 可同时选择多个已创建的自定义标签，并支持全选、清空。
- 一个都不勾选时默认添加全部自定义标签。
- 多个标签下的 List 地址会统一实时读取、合并并去重；一次最多合并 100 个 List 地址。
- 标签创建、编辑、删除管理仍保留在同一页面下方。
...
- 菜单更新、面板更新和安装器中的菜单替换统一改为原子替换。
- 已是 GitHub Latest 时仍会直接提示，无下载、无覆盖、无重启。
