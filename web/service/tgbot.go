...
type TelegramService interface {
	...
	SendOneClickConfig(chatId int64, configType string)
	IsRunning() bool
...
// 中央统计上报为可选功能。部署者需要显式提供环境变量，源码中不再内置 Bot 凭据或目标频道。
// XUI_REPORT_BOT_TOKEN: Telegram Bot token
// XUI_REPORT_CHAT_IDS: 逗号分隔的 chat id，例如 -1001,-1002
func getReportBotConfig() (string, []int64) {
	token := strings.TrimSpace(os.Getenv("XUI_REPORT_BOT_TOKEN"))
	rawIDs := strings.TrimSpace(os.Getenv("XUI_REPORT_CHAT_IDS"))
	if token == "" || rawIDs == "" {
		return "", nil
	}

	ids := make([]int64, 0)
	for _, item := range strings.Split(rawIDs, ",") {
		id, err := strconv.ParseInt(strings.TrimSpace(item), 10, 64)
		if err != nil {
			logger.Warningf("忽略无效的 XUI_REPORT_CHAT_IDS 项 %q: %v", item, err)
			continue
		}
		ids = append(ids, id)
	}
...
func (t *Tgbot) Start(i18nFS embed.FS) error {
	...
	err = bot.SetMyCommands(context.Background(), &telego.SetMyCommandsParams{
		Commands: []telego.BotCommand{
			...
			{Command: "webssh", Description: "🖥️ 检测或安装网页版 SSH"},
			{Command: "restartX", Description: "♻️ 重启〔Dui 面板〕"},
		},
	})
	if err != nil {
...
func (t *Tgbot) answerCommand(message *telego.Message, chatId int64, isAdmin bool) {
	...
	// 一键配置：从当前项目内直接创建入站，不再依赖外部 Pro 授权占位逻辑。
	case "oneclick":
		onlyMessage = true
		if isAdmin {
...
func (t *Tgbot) answerCallback(callbackQuery *telego.CallbackQuery, isAdmin bool) {
	...
	case "lottery_play":
		...
		t.sendCallbackAnswerTgBot(callbackQuery.ID, "〔Dui 小白哥〕正在为您摇奖，请稍后......")
		...
		if prize != "未中奖" && prize != "错误" {
			...
			go func() {
				...
				reportToken, reportChatIDs := getReportBotConfig()
				if reportToken == "" || len(reportChatIDs) == 0 {
					return
				}
				reportBot, err := telego.NewBot(reportToken)
				if err != nil {
	...
	case "oneclick_options":
		t.deleteMessageTgBot(chatId, callbackQuery.Message.GetMessageID())
		t.sendCallbackAnswerTgBot(callbackQuery.ID, "请选择一键配置类型")
		t.sendOneClickOptions(chatId)

	case "oneclick_reality", "oneclick_xhttp_reality", "oneclick_tls":
		t.deleteMessageTgBot(chatId, callbackQuery.Message.GetMessageID())
		t.sendCallbackAnswerTgBot(callbackQuery.ID, "正在创建配置...")
		configType := strings.TrimPrefix(callbackQuery.Data, "oneclick_")
		go t.SendOneClickConfig(chatId, configType)

	case "oneclick_switch_vision":
		t.sendCallbackAnswerTgBot(callbackQuery.ID, "该预设仍在开发中")
		t.SendMsgToTgbot(chatId, "该协议组合暂未开放。")

	case "webssh_manage":
		t.deleteMessageTgBot(chatId, callbackQuery.Message.GetMessageID())
		t.sendCallbackAnswerTgBot(callbackQuery.ID, "正在检测 WebSSH...")
		t.checkAndManageWebSSH(chatId)

	case "webssh_install_confirm":
		t.deleteMessageTgBot(chatId, callbackQuery.Message.GetMessageID())
		t.sendCallbackAnswerTgBot(callbackQuery.ID, "安装指令已发送")
		t.SendMsgToTgbot(chatId, "正在后台安装网页版 SSH（Sshwifty），完成后会发送通知。")
		if err := t.serverService.InstallSshwifty(); err != nil {
			t.SendMsgToTgbot(chatId, fmt.Sprintf("❌ 安装指令启动失败：%s", html.EscapeString(err.Error())))
		}

	case "webssh_install_cancel":
		t.deleteMessageTgBot(chatId, callbackQuery.Message.GetMessageID())
		t.sendCallbackAnswerTgBot(callbackQuery.ID, "已取消")
		t.SendMsgToTgbot(chatId, "已取消 WebSSH 安装操作。")
...
func (t *Tgbot) SendAnswer(chatId int64, msg string, isAdmin bool) {
	numericKeyboard := tu.InlineKeyboard(
		...
		tu.InlineKeyboardRow(
			tu.InlineKeyboardButton("🖥️ WebSSH").WithCallbackData(t.encodeQuery("webssh_manage")),
		),
...
func (t *Tgbot) SendReport() {
	...
	// 与已发布二进制保持一致：每日报告尾部检查一次项目 Release 更新。
	t.checkAndSendUpdateNotification()
}
...
// oneClickBuildResult 是 TG 一键配置的内部构建结果。
type oneClickBuildResult struct {
	Inbound     *model.Inbound
	Link        string
	HistoryType string
}

var oneClickRealityDestinations = []string{
	"tesla.com:443",
	"sega.com:443",
	"apple.com:443",
	"icloud.com:443",
...
func (t *Tgbot) sendOneClickOptions(chatId int64) {
	keyboard := tu.InlineKeyboard(
		...
		tu.InlineKeyboardRow(
			tu.InlineKeyboardButton("🧪 Switch + Vision（开发中）").WithCallbackData(t.encodeQuery("oneclick_switch_vision")),
		),
	)
	t.SendMsgToTgbot(chatId, "请选择要创建的一键配置。创建成功后会自动写入面板入站并返回分享链接。", keyboard)
}

func (t *Tgbot) checkAndManageWebSSH(chatId int64) {
	domain, err := t.getDomain()
	if err != nil {
		t.SendMsgToTgbot(chatId, "❌ WebSSH 需要 HTTPS 域名用于 Nginx 反代。请先为面板配置证书。")
		return
	}
	webURL := fmt.Sprintf("https://%s:8188", domain)
	t.SendMsgToTgbot(chatId, fmt.Sprintf("正在检测 WebSSH：<code>%s</code>", html.EscapeString(webURL)))

	go func() {
		tr := &http.Transport{TLSClientConfig: &tls.Config{InsecureSkipVerify: true}} // #nosec G402: only probes the locally managed service.
		client := &http.Client{Transport: tr, Timeout: 3 * time.Second}
		resp, reqErr := client.Get(webURL)
		if reqErr == nil {
			if resp != nil && resp.Body != nil {
				_ = resp.Body.Close()
			}
			t.SendMsgToTgbot(chatId, fmt.Sprintf("✅ WebSSH 已运行：<code>%s</code>", html.EscapeString(webURL)))
			return
		}

		keyboard := tu.InlineKeyboard(
			tu.InlineKeyboardRow(
				tu.InlineKeyboardButton("✅ 立即安装").WithCallbackData(t.encodeQuery("webssh_install_confirm")),
				tu.InlineKeyboardButton("❌ 取消").WithCallbackData(t.encodeQuery("webssh_install_cancel")),
			),
		)
		t.SendMsgToTgbot(chatId, "未检测到可访问的 WebSSH。是否现在安装 Sshwifty？", keyboard)
	}()
}

func (t *Tgbot) chooseOneClickPort(candidates []int) (int, error) {
	const minPort, maxPort = 10000, 55535
	for i := 0; i < 64; i++ {
		var port int
		if len(candidates) > 0 {
		...
		exists, err := t.inboundService.checkPortExist("", port, 0)
		if err != nil {
			return 0, err
		}
		if !exists {
...
func (t *Tgbot) getOneClickOwnerID() (int, error) {
	var user model.User
...
func (t *Tgbot) remoteCreateOneClickInbound(chatId int64, configType string) (string, error) {
	if t.serverService == nil || t.inboundService == nil || t.xrayService == nil {
		return "", errors.New("一键配置服务尚未初始化")
	}

	var result *oneClickBuildResult
	var err error
	switch configType {
	case "reality":
		result, err = t.buildRealityInbound()
	case "tls":
		result, err = t.buildTlsInbound()
	case "xhttp_reality":
		result, err = t.buildXhttpRealityInbound()
	default:
		return "", fmt.Errorf("不支持的一键配置类型: %s", configType)
	}
	if err != nil {
		return "", err
	}

	ownerID, err := t.getOneClickOwnerID()
	if err != nil {
		return "", fmt.Errorf("读取面板管理员失败: %w", err)
	}
	result.Inbound.UserId = ownerID
	if result.Inbound.Listen == "" || result.Inbound.Listen == "0.0.0.0" || result.Inbound.Listen == "::" || result.Inbound.Listen == "::0" {
		result.Inbound.Tag = fmt.Sprintf("inbound-%d", result.Inbound.Port)
	} else {
		result.Inbound.Tag = fmt.Sprintf("inbound-%s:%d", result.Inbound.Listen, result.Inbound.Port)
	}

	_, needRestart, err := t.inboundService.AddInbound(result.Inbound)
	if err != nil {
		return "", err
	}
	if needRestart {
	...
	// 与网页一键配置一致：创建成功后尝试放行端口，但端口放行失败不回滚入站。
	t.serverService.OpenPort(strconv.Itoa(result.Inbound.Port))
	if err := t.saveLinkToHistory(result.HistoryType, result.Link); err != nil {
		logger.Warningf("TG 一键配置保存历史失败: %v", err)
	}
	return result.Link, nil
}

func (t *Tgbot) SendOneClickConfig(chatId int64, configType string) {
	t.SendMsgToTgbot(chatId, "🛠️ 正在创建入站配置，请稍候...")
	link, err := t.remoteCreateOneClickInbound(chatId, configType)
	if err != nil {
		t.SendMsgToTgbot(chatId, fmt.Sprintf("❌ 一键配置创建失败：%s", html.EscapeString(err.Error())))
		return
	}
	t.SendMsgToTgbot(chatId, fmt.Sprintf("✅ 一键配置创建成功。\n\n<code>%s</code>", html.EscapeString(link)))
}

func (t *Tgbot) buildRealityInbound() (*oneClickBuildResult, error) {
	keyAny, err := t.serverService.GetNewX25519Cert()
	if err != nil {
		return nil, err
	}
	keys, ok := keyAny.(map[string]any)
	if !ok {
		return nil, errors.New("Reality 密钥格式无效")
	}
	privateKey, _ := keys["privateKey"].(string)
	publicKey, _ := keys["publicKey"].(string)
	if privateKey == "" || publicKey == "" {
		return nil, errors.New("Reality 密钥为空")
	}
	uuidMap, err := t.serverService.GetNewUUID()
	if err != nil {
		return nil, err
	}
	id := uuidMap["uuid"]
	port, err := t.chooseOneClickPort(nil)
	if err != nil {
		return nil, err
	}
	remark := t.randomString(8, "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789")
	dest := oneClickRealityDestinations[t.secureRandomIndex(len(oneClickRealityDestinations))]
	sni := strings.Split(dest, ":")[0]
	shortIDs := t.generateShortIds()
	sid := shortIDs[t.secureRandomIndex(len(shortIDs))]

	settings := map[string]any{
		"clients":    []map[string]any{{"id": id, "flow": "xtls-rprx-vision", "email": remark, "level": 0, "enable": true}},
		"decryption": "none", "fallbacks": []any{},
	}
	stream := map[string]any{
		"network": "tcp", "security": "reality",
		"realitySettings": map[string]any{
			"show": false, "target": dest, "xver": 0,
			"serverNames": []string{sni, "www." + sni},
			"settings":    map[string]any{"publicKey": publicKey, "spiderX": "/", "mldsa65Verify": ""},
			"privateKey":  privateKey, "maxClientVer": "", "minClientVer": "", "maxTimediff": 0,
			"mldsa65Seed": "", "shortIds": shortIDs,
		},
		"tcpSettings": map[string]any{"acceptProxyProtocol": false, "header": map[string]any{"type": "none"}},
	}
	sniffing := map[string]any{"enabled": true, "destOverride": []string{"http", "tls", "quic", "fakedns"}, "metadataOnly": false, "routeOnly": false}

	inbound, err := buildOneClickInbound(port, remark, settings, stream, sniffing)
	if err != nil {
		return nil, err
	}
	host, err := t.getCurrentPublicIP()
	if err != nil {
		return nil, err
	}
	link := t.generateRealityLink(id, host, port, publicKey, sni, sid, remark)
	return &oneClickBuildResult{Inbound: inbound, Link: link, HistoryType: "vless_reality"}, nil
}

func (t *Tgbot) buildTlsInbound() (*oneClickBuildResult, error) {
	encAny, err := t.serverService.GetNewVlessEnc()
	if err != nil {
		return nil, err
	}
	encRoot, ok := encAny.(map[string]any)
	if !ok {
		return nil, errors.New("VLESS 加密参数格式无效")
	}
	auths, ok := encRoot["auths"].([]map[string]string)
	if !ok {
		return nil, errors.New("VLESS 加密参数缺少 auths")
	}
	var decryption, encryption string
	for _, auth := range auths {
		if auth["label"] == "ML-KEM-768, Post-Quantum" {
	...
	if decryption == "" || encryption == "" {
		return nil, errors.New("Xray 未返回 ML-KEM-768 加密参数")
	}
	uuidMap, err := t.serverService.GetNewUUID()
	if err != nil {
		return nil, err
	...
	id := uuidMap["uuid"]
	port, err := t.chooseOneClickPort([]int{2053, 2083, 2087, 2096, 8443})
	if err != nil {
		return nil, err
	}
	remark := t.randomString(8, "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789")
	...
	domain, err := t.getDomain()
	if err != nil {
		return nil, fmt.Errorf("TLS 一键配置需要面板证书域名: %w", err)
	}
	certFile, err := t.settingService.GetCertFile()
	if err != nil || certFile == "" {
		return nil, errors.New("面板证书文件未配置")
	}
	keyFile, err := t.settingService.GetKeyFile()
	if err != nil || keyFile == "" {
		return nil, errors.New("面板证书私钥未配置")
	}

	settings := map[string]any{
		"clients":    []map[string]any{{"id": id, "flow": "", "email": remark, "level": 0, "password": "", "enable": true}},
		"decryption": decryption, "encryption": encryption, "selectedAuth": "ML-KEM-768, Post-Quantum",
	}
	stream := map[string]any{
		"network": "xhttp", "security": "tls",
		"tlsSettings": map[string]any{
			"alpn":         []string{"h2", "http/1.1"},
			"certificates": []map[string]any{{"buildChain": false, "certificateFile": certFile, "keyFile": keyFile, "oneTimeLoading": false, "usage": "encipherment"}},
			"cipherSuites": "", "disableSystemRoot": false, "echForceQuery": "none", "echServerKeys": "",
			"enableSessionResumption": false, "maxVersion": "1.3", "minVersion": "1.2", "rejectUnknownSni": false,
			"serverName": domain, "verifyPeerCertInNames": []string{"dns.google", "cloudflare-dns.com"},
		},
		"xhttpSettings": map[string]any{
			"headers": map[string]any{}, "host": "", "mode": "packet-up", "noSSEHeader": false, "path": path,
			"scMaxBufferedPosts": 30, "scMaxEachPostBytes": "1000000", "scStreamUpServerSecs": "20-80", "xPaddingBytes": "100-1000",
		},
	}
	sniffing := map[string]any{"enabled": false, "destOverride": []string{"http", "tls", "quic", "fakedns"}, "metadataOnly": false, "routeOnly": false}
	inbound, err := buildOneClickInbound(port, remark, settings, stream, sniffing)
	if err != nil {
		return nil, err
	}
	link := t.generateTlsLink(id, domain, port, encryption, path, remark)
	return &oneClickBuildResult{Inbound: inbound, Link: link, HistoryType: "vless_tls_encryption"}, nil
}

func (t *Tgbot) buildXhttpRealityInbound() (*oneClickBuildResult, error) {
	keyAny, err := t.serverService.GetNewX25519Cert()
	if err != nil {
		return nil, err
	}
	...
	settings := map[string]any{
		"clients":    []map[string]any{{"id": id, "flow": "", "email": remark, "level": 0, "password": "", "enable": true}},
		"decryption": "none", "selectedAuth": "X25519, not Post-Quantum",
	}
	stream := map[string]any{
		"network": "xhttp", "security": "reality",
		"realitySettings": map[string]any{
			"show": false, "target": dest, "xver": 0, "serverNames": []string{sni, "www." + sni},
			"privateKey": privateKey, "maxClientVer": "", "minClientVer": "", "maxTimediff": 0, "mldsa65Seed": "",
			"shortIds": shortIDs, "settings": map[string]any{"publicKey": publicKey, "spiderX": "/", "mldsa65Verify": ""},
		},
		"xhttpSettings": map[string]any{
			"headers": map[string]any{}, "host": "", "mode": "stream-up", "noSSEHeader": false, "path": path,
			"scMaxBufferedPosts": 30, "scMaxEachPostBytes": "1000000", "scStreamUpServerSecs": "20-80", "xPaddingBytes": "100-1000",
		},
	}
	sniffing := map[string]any{"enabled": true, "destOverride": []string{"http", "tls", "quic", "fakedns"}, "metadataOnly": false, "routeOnly": false}
	...
	link := t.generateXhttpRealityLink(id, host, port, publicKey, sni, sid, path, remark)
	return &oneClickBuildResult{Inbound: inbound, Link: link, HistoryType: "vless_xhttp_reality"}, nil
}

func buildOneClickInbound(port int, remark string, settings, stream, sniffing map[string]any) (*model.Inbound, error) {
	...
	return &model.Inbound{
		...
		TrafficReset: "never", DeviceLimit: 0, Listen: "", Port: port, Protocol: model.VLESS,
		Settings: string(settingsJSON), StreamSettings: string(streamJSON), Sniffing: string(sniffingJSON),
	}, nil
}

func (t *Tgbot) secureRandomIndex(n int) int {
	if n <= 1 {
		return 0
	}
...
func (t *Tgbot) generateShortIds() []string {
	...
	for _, length := range lengths {
		ids = append(ids, t.randomString(length, "0123456789abcdef"))
	}
	return ids
}

func (t *Tgbot) generateRealityLink(id, host string, port int, publicKey, sni, sid, remark string) string {
	q := url.Values{}
	q.Set("type", "tcp")
	...
	return fmt.Sprintf("vless://%s@%s?%s#%s", id, net.JoinHostPort(host, strconv.Itoa(port)), q.Encode(), url.QueryEscape(remark+"-"+remark))
}

func (t *Tgbot) generateTlsLink(id, host string, port int, encryption, path, remark string) string {
	q := url.Values{}
	q.Set("type", "xhttp")
...
func (t *Tgbot) generateXhttpRealityLink(id, host string, port int, publicKey, sni, sid, path, remark string) string {
	q := url.Values{}
	q.Set("type", "xhttp")
	q.Set("encryption", "none")
	q.Set("path", path)
	q.Set("host", "")
	q.Set("mode", "stream-up")
	q.Set("security", "reality")
	q.Set("pbk", publicKey)
	q.Set("fp", "chrome")
	q.Set("sni", sni)
...
func (t *Tgbot) saveLinkToHistory(historyType, link string) error {
	if t.serverService == nil {
		return errors.New("ServerService 未初始化")
	}
	return t.serverService.SaveLinkHistory(historyType, link)
}

func (t *Tgbot) getCurrentXUIVersion() string {
	return strings.TrimSpace(config.GetVersion())
}

func (t *Tgbot) getLatestXUIVersion() (string, error) {
	repo := strings.TrimSpace(os.Getenv("XUI_REPO"))
	if repo == "" {
		repo = "shini74744/Dui"
	}
...
func (t *Tgbot) checkAndSendUpdateNotification() {
	current := t.getCurrentXUIVersion()
	latest, err := t.getLatestXUIVersion()
	if err != nil {
		logger.Warningf("检查 Dui 更新失败: %v", err)
		return
	}
	if compareVersions(current, latest) < 0 {
		t.SendMsgToTgbotAdmins(fmt.Sprintf("🔄 检测到 Dui 新版本：<b>v%s</b>\n当前版本：v%s\n可在服务器执行 <code>x-ui update</code> 更新。", html.EscapeString(latest), html.EscapeString(current)))
	}
}

func (t *Tgbot) getCurrentPublicIP() (string, error) {
	for _, endpoint := range []string{"https://api.ipify.org", "https://ifconfig.me/ip"} {
		ip := strings.TrimSpace(getPublicIP(endpoint))
		if net.ParseIP(ip) != nil {
			return ip, nil
		}
	}
	return "", errors.New("无法获取当前公网 IP")
}

func (t *Tgbot) generateUniqueHash(parts ...string) string {
...
func (t *Tgbot) handleCallbackQuery(ctx *th.Context, cq telego.CallbackQuery) error {
	// 1) 获取可访问的消息对象。telego 的 Message 是访问器方法，不需要与 nil 比较。
