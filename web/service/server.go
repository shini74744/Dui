...
// InstallSshwifty runs the Sshwifty installer exposed by the x-ui menu script.
// The installer is intentionally asynchronous so the HTTP request does not time out.
func (s *ServerService) InstallSshwifty() error {
	const scriptPath = "/usr/bin/x-ui"
	if _, err := os.Stat(scriptPath); err != nil {
		return fmt.Errorf("Sshwifty 安装失败：找不到 %s: %w", scriptPath, err)
	}

	go func() {
		cmd := exec.Command(scriptPath, "sshwifty")
		output, err := cmd.CombinedOutput()
		if err != nil {
			logger.Errorf("Sshwifty 安装失败: %v\n输出: %s", err, string(output))
			if s.tgService != nil && s.tgService.IsRunning() {
				_ = s.tgService.SendMessage(fmt.Sprintf("❌ **网页版SSH安装失败**\n\n错误: %v", err))
			}
			return
		}
		logger.Infof("Sshwifty 安装完成: %s", string(output))
		if s.tgService != nil && s.tgService.IsRunning() {
			_ = s.tgService.SendMessage("✅ **网页版SSH（Sshwifty）安装完成**")
		}
	}()
	return nil
}
