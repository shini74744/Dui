#!/usr/bin/env bash
# ============================================================
# Fail2ban SSH Protector 菜单版 + Dui 扩展版（nftables 修复版）
#
# 说明：
#   - SSH / 3xui-tls / 3xui-login 三套规则完全独立
#   - Dui 两套 jail 仅针对 nftables 环境实现“被 ban 后整机拦”
#   - Dui 两套规则依赖 systemd journal（backend = systemd）
# ============================================================
...
REMOTE_URL="https://raw.githubusercontent.com/shini74744/Dui/main/web/service/fb5.sh"
...
# Dui 整机拦：同时支持 nftables 和 iptables
...
prompt_xui_port() {
    local p=""
    while true; do
        read -rp "请输入 Dui 面板端口号（必填，例如 744 / 2053 / 54321）: " p
        if [[ "$p" =~ ^[0-9]+$ ]] && (( p >= 1 && p <= 65535 )); then
            echo "$p"
            return
        fi
...
ensure_xui_fail2ban_env() {
    ...
    if ! command -v systemctl &>/dev/null; then
        echo "❌ Dui 日志封禁功能依赖 systemd journal，当前系统未检测到 systemctl。"
        return 1
    fi
    ...
    if [[ "$FIREWALL" != "nftables" && "$FIREWALL" != "iptables" ]]; then
        ...
        echo "   Dui『被 ban 后整机拦』功能目前只支持 nftables / iptables。"
        return 1
    fi
...
#-----------------------------
get_banned_ips_for_jail() {
    local jail_name="$1"
    local ips=""
    local status_out=""

    if ips="$(fail2ban-client get "$jail_name" banip 2>/dev/null)"; then
    ...
    status_out="$(fail2ban-client status "$jail_name" 2>/dev/null || true)"
    echo "$status_out" | sed -n 's/.*Banned IP list:[[:space:]]*//p' | tail -n1
}

show_recent_attack_logs_for_jail() {
    local jail_name="$1"
    local lines="${2:-100}"

    echo "================ ${jail_name} 最近爆破日志 ================"
    if [[ -f /var/log/fail2ban.log ]]; then
        grep -E "\\[${jail_name}\\][[:space:]]+(Found|Ban|Unban|Restore Ban)[[:space:]]" /var/log/fail2ban.log 2>/dev/null | tail -n "$lines" || true
    elif command -v journalctl &>/dev/null; then
        journalctl -u fail2ban -n 500 --no-pager 2>/dev/null | grep -E "\\[${jail_name}\\].*(Found|Ban|Unban|Restore Ban)" | tail -n "$lines" || true
    else
        echo "⚠ 未找到 /var/log/fail2ban.log，也无法使用 journalctl。"
    fi
    echo "=========================================================="
    echo ""
}
...
view_banned_ips_for_jail() {
    ...
    local ips=""
    ips="$(get_banned_ips_for_jail "$jail_name")"
    local count=0
    if [[ -n "$ips" ]]; then
        count="$(echo "$ips" | wc -w | tr -d ' ')"
    fi

    echo "================ ${title} 当前封禁 IP（${count}） ================"
    if [[ -z "$ips" ]]; then
        echo "✅ 当前无封禁 IP"
    else
        echo "$ips" | tr ' ' '\n'
    ...
    echo "=============================================================="
    echo ""
    echo "提示：需要取消拉黑可返回主菜单使用对应“解禁指定 IP”选项。"
    echo ""
    pause
}
...
unban_ip_for_jail() {
    ...
    local ip=""
    read -rp "请输入要从 ${title} 取消拉黑的 IP（IPv4/IPv6，回车取消）: " ip
    if [[ -z "$ip" ]]; then
        echo "已取消。"
        pause
        return
...
view_sshd_attack_logs() {
    if ! ensure_jail_ready "sshd"; then pause; return; fi
    show_recent_attack_logs_for_jail "sshd" 100
    pause
}

view_3xui_tls_attack_logs() {
    if ! ensure_jail_ready "3xui-tls"; then pause; return; fi
    show_recent_attack_logs_for_jail "3xui-tls" 100
    pause
}

view_3xui_login_attack_logs() {
    if ! ensure_jail_ready "3xui-login"; then pause; return; fi
    show_recent_attack_logs_for_jail "3xui-login" 100
    pause
}
...
#-----------------------------
view_banned_ips() {
    view_banned_ips_for_jail "sshd" "sshd"
}
...
print_status_summary() {
    ...
    local ssh_banned_count=0
    local tls_banned_count=0
    local login_banned_count=0
    if command -v fail2ban-client &>/dev/null && [[ "$fb_status" == "运行中" ]]; then
        local _ips=""
        _ips="$(get_banned_ips_for_jail sshd 2>/dev/null || true)"; [[ -n "$_ips" ]] && ssh_banned_count="$(echo "$_ips" | wc -w | tr -d ' ')"
        _ips="$(get_banned_ips_for_jail 3xui-tls 2>/dev/null || true)"; [[ -n "$_ips" ]] && tls_banned_count="$(echo "$_ips" | wc -w | tr -d ' ')"
        _ips="$(get_banned_ips_for_jail 3xui-login 2>/dev/null || true)"; [[ -n "$_ips" ]] && login_banned_count="$(echo "$_ips" | wc -w | tr -d ' ')"
    fi
    echo "Dui TLS 扫描封禁: $xui_tls_jail (记录端口: $xui_tls_port, 当前拉黑: $tls_banned_count, 被 ban 后整机拦, 防火墙: $FIREWALL)"
    echo "Dui 登录失败封禁: $xui_login_jail (记录端口: $xui_login_port, 当前拉黑: $login_banned_count, 被 ban 后整机拦, 防火墙: $FIREWALL)"
    echo "SSH 当前拉黑: $ssh_banned_count"
...
#-----------------------------
# 7. 启用 / 配置 Dui TLS 扫描封禁（被 ban 后整机拦）
#-----------------------------
enable_3xui_tls_protection() {
    ...
    echo "🛡 正在启用 / 更新 Dui TLS 异常扫描封禁（被 ban 后整机拦）..."
...
#-----------------------------
# 8. 修改 Dui TLS 扫描封禁参数（可改端口）
#-----------------------------
modify_3xui_tls_params() {
    if [[ ! -f "$XUI_TLS_JAIL" ]]; then
        echo "⚠ 未检测到 $XUI_TLS_JAIL，请先执行『7) 启用 / 配置 Dui TLS 扫描封禁』"
        pause
        return
    fi
    ...
    echo "✅ Dui TLS 扫描封禁参数已更新。"
    show_jail_simple_status "3xui-tls"
    pause
}

#-----------------------------
# 9. 查看 Dui TLS 扫描封禁 IP 列表
#-----------------------------
...
#-----------------------------
# 11. 启用 / 配置 Dui 登录失败封禁（被 ban 后整机拦）
#-----------------------------
enable_3xui_login_protection() {
    ...
    echo "🛡 正在启用 / 更新 Dui 面板登录失败封禁（被 ban 后整机拦）..."
    echo "   记录端口: $PANEL_PORT"
    echo "   动作: $ACTION"
    echo "   白名单: $IGNOREIPS"
    echo "   maxretry: $CUR_MAXRETRY"
    ...
    echo "⚠ 注意：请确认 Dui 失败登录日志里的 IP 是真实来访 IP。"
    show_jail_simple_status "3xui-login"
    pause
}

#-----------------------------
# 12. 修改 Dui 登录失败封禁参数（可改端口）
#-----------------------------
modify_3xui_login_params() {
    if [[ ! -f "$XUI_LOGIN_JAIL" ]]; then
        echo "⚠ 未检测到 $XUI_LOGIN_JAIL，请先执行『11) 启用 / 配置 Dui 登录失败封禁』"
        pause
        return
    fi
    ...
    echo "✅ Dui 登录失败封禁参数已更新。"
    show_jail_simple_status "3xui-login"
    pause
}

#-----------------------------
# 13. 查看 Dui 登录失败封禁 IP 列表
#-----------------------------
...
uninstall_all() {
    while true; do
        ...
        echo ""
        echo " 1) 卸载全部配置（SSH + Dui TLS + Dui 登录失败 + 自定义 action）"
        echo " 2) 仅卸载 SSH 防爆破配置（只删除 jail.local 中的 [sshd] 段）"
        echo " 3) 仅卸载 Dui TLS 扫描封禁配置"
        echo " 4) 仅卸载 Dui 登录失败封禁配置"
        echo " 5) 仅卸载 Dui 自定义 nftables action"
        ...
        case "$UCHOICE" in
            ...
            4)
                ...
                ;;
            5)
                echo "⚠ 将仅删除 Dui 自定义 nftables action："
                echo "   - $XUI_NFT_ALL_ACTION"
                read -rp "确认继续吗？[y/N]: " CONFIRM
                case "$CONFIRM" in
                    y|Y)
                        rm -f "$XUI_NFT_ALL_ACTION"
                        echo "✅ 已删除 Dui 自定义 nftables action。"
                        restart_fail2ban_if_present
                        ;;
                    *)
                        echo "已取消。"
...
main_menu() {
    while true; do
        clear
        echo "=========================================================="
        echo " Fail2ban SSH / Dui 防爆破 管理脚本（nftables 修复版）"
        echo "=========================================================="
        print_status_summary
        ...
        echo " 6) 解禁指定 IP（sshd）"
        echo " 7) 启用 / 配置 Dui TLS 扫描封禁（被 ban 后整机拦）"
        echo " 8) 修改 Dui TLS 扫描封禁参数（可改端口）"
        echo " 9) 查看 Dui TLS 扫描封禁 IP 列表"
        echo "10) 解禁指定 IP（3xui-tls）"
        echo "11) 启用 / 配置 Dui 登录失败封禁（被 ban 后整机拦）"
        echo "12) 修改 Dui 登录失败封禁参数（可改端口）"
        echo "13) 查看 Dui 登录失败封禁 IP 列表"
        echo "14) 解禁指定 IP（3xui-login）"
        echo "15) 查看 sshd 最近爆破日志"
        echo "16) 查看 Dui TLS 最近爆破日志"
        echo "17) 查看 Dui 登录最近爆破日志"
        echo " 0) 退出"
        echo "----------------------------------------------------------"
        ...
        case "$CHOICE" in
            ...
            15) view_sshd_attack_logs ;;
            16) view_3xui_tls_attack_logs ;;
            17) view_3xui_login_attack_logs ;;
            0) echo "已退出。"; exit 0 ;;
            *) echo "❌ 无效选项。"; pause ;;
        esac
    done
