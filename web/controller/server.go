...
type swapApplyForm struct {
	TotalMB    int64 `json:"totalMB" form:"totalMB"`
	Swappiness int   `json:"swappiness" form:"swappiness"`
}
...
func (a *ServerController) initRouter(g *gin.RouterGroup) {
	...
	g.POST("/install/sshwifty", a.installSshwifty)
...
func (a *ServerController) getSwapStatus(c *gin.Context) {
	status, err := a.swapService.Status()
	jsonObj(c, status, err)
}

func (a *ServerController) applySwap(c *gin.Context) {
	form := &swapApplyForm{}
	if err := c.ShouldBind(form); err != nil {
		jsonMsg(c, "Swap 参数无效", err)
		return
	}
	err := a.swapService.Apply(service.SwapApplyOptions{TotalMB: form.TotalMB, Swappiness: form.Swappiness})
	jsonMsg(c, "Swap 设置已应用", err)
	if err == nil {
		a.refreshStatus()
	}
}
...
// installSshwifty starts the existing x-ui sshwifty installer in the background.
func (a *ServerController) installSshwifty(c *gin.Context) {
	if err := a.serverService.InstallSshwifty(); err != nil {
		jsonMsg(c, "Sshwifty 安装指令执行失败", err)
		return
	}
	jsonMsg(c, "Sshwifty 安装指令已成功发送", nil)
}
