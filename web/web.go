...
func (s *Server) initRouter() (*gin.Engine, error) {
	...
	engine.Use(sessions.Sessions("Dui", store))
...
func (s *Server) startTask() {
	...
	// Reset traffic for inbounds that opt into an automatic period.
	s.cron.AddJob("@daily", job.NewPeriodicTrafficResetJob(job.Daily))
	s.cron.AddJob("@weekly", job.NewPeriodicTrafficResetJob(job.Weekly))
	s.cron.AddJob("@monthly", job.NewPeriodicTrafficResetJob(job.Monthly))
