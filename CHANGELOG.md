...
- 旧路由现在可以根据现有域名/IP 内容反向识别以前使用过的自定义标签。
- 打开历史路由时会实时解析已保存自定义标签下的 List，并与当前路由内容做可靠匹配；确认命中后自动勾选对应标签。
- 小规则集要求完整匹配；较大规则集要求至少命中 3 条且覆盖率约 80% 以上，降低相似规则误判风险。
- 支持识别旧规则只添加域名、只添加 IP/CIDR/GeoIP，或域名 + IP 同时添加的情况。
- 识别成功后会立即保存来源映射，后续再次编辑无需重新猜测。
- 新增路由来源侧边元数据文件 `/etc/x-ui/route_rule_sources.json`，不写入 Xray JSON，不影响 Xray 原生配置。
- 新建/编辑规则时会记录所选自定义标签；规则内容变化时来源映射自动迁移到新指纹。
- 删除路由后会在后续保存时清理失效来源记录；删除自定义标签时也会从来源映射中自动移除对应 ID。
- 来源映射会过滤不存在的自定义标签和无效指纹，避免长期积累脏数据。

### 验证

- 新增路由来源映射持久化、标签删除联动清理和历史匹配阈值回归测试。
- 保持现有多 List 标签、多选/全选、私网 URL 拒绝、旧单地址迁移等测试通过。
