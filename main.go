...
func main() {
	...
	certManagerCmd := flag.NewFlagSet("cert-manager-record", flag.ExitOnError)
	var certManagerSource string
	var certManagerDomain string
	var certManagerCertFile string
	var certManagerKeyFile string
	var certManagerWildcard bool
	var certManagerIssuePort int
	certManagerCmd.StringVar(&certManagerSource, "source", "", "certificate source")
	certManagerCmd.StringVar(&certManagerDomain, "domain", "", "certificate domain")
	certManagerCmd.StringVar(&certManagerCertFile, "cert", "", "certificate fullchain path")
	certManagerCmd.StringVar(&certManagerKeyFile, "key", "", "certificate private key path")
	certManagerCmd.BoolVar(&certManagerWildcard, "wildcard", false, "wildcard certificate")
	certManagerCmd.IntVar(&certManagerIssuePort, "issuePort", 0, "standalone issue port")

	certTouchCmd := flag.NewFlagSet("cert-manager-touch", flag.ExitOnError)
	var certTouchDomain string
	certTouchCmd.StringVar(&certTouchDomain, "domain", "", "renewed certificate domain")
	...
	flag.Usage = func() {
		...
		fmt.Println("    cert-manager-record  record menu 18/19 certificate metadata")
		fmt.Println("    cert-manager-touch   mark managed certificate renewed")
		fmt.Println("    cert-renew-check     run managed certificate due check")
	}
	...
	case "cert-manager-record":
		if err := certManagerCmd.Parse(os.Args[2:]); err != nil {
			fmt.Println(err)
			return
		}
		svc := service.CertManagerService{}
		if err := svc.RecordFromMenu(certManagerSource, certManagerDomain, certManagerCertFile, certManagerKeyFile, certManagerWildcard, certManagerIssuePort); err != nil {
			fmt.Println("record certificate metadata failed:", err)
			os.Exit(1)
		}
		fmt.Println("certificate metadata recorded")
	case "cert-manager-touch":
		if err := certTouchCmd.Parse(os.Args[2:]); err != nil {
			fmt.Println(err)
			return
		}
		svc := service.CertManagerService{}
		if err := svc.RecordRenewedNow(certTouchDomain); err != nil {
			fmt.Println("touch certificate metadata failed:", err)
			os.Exit(1)
		}
	case "cert-renew-check":
		svc := service.CertManagerService{}
		out, err := svc.CheckDueAndRenew()
		if out != "" {
			fmt.Println(out)
		}
		if err != nil {
			fmt.Println("certificate renewal check failed:", err)
			os.Exit(1)
		}
	default:
